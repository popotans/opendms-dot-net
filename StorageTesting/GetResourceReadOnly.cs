...
{
    public class GetResourceReadOnly : TestBase
    {
        private DateTime _start;

        public GetResourceReadOnly(FrmMain window, IEngine engine, IDatabase db)
        ...
        void win_OnGoClick(string id)
        {
            ...
            WriteLine("Starting GetResourceReadOnly test...");
            _start = DateTime.Now;
            _engine.GetResourceReadOnly(request, new OpenDMS.Storage.Data.ResourceId(id));
        }

        private void AuthorizationDenied(EngineRequest request)
        {
            WriteLine("GetResourceReadOnly.AuthorizationDenied - Access to the resource was denied based on usage permissions.");
        }

        private void EngineAction(EngineRequest request, EngineActionType actionType, bool willSendProgress)
        {
            if (willSendProgress)
                WriteLine("GetResourceReadOnly.EngineAction - Type: " + actionType.ToString() + " Expecting Progress Reports.");
            else
                WriteLine("GetResourceReadOnly.EngineAction - Type: " + actionType.ToString() + " NOT Expecting Progress Reports.");
        }
        ...
        {
            WriteLine("GetResourceReadOnly.Progress - Sent: " + sendPercentComplete.ToString() + " Received: " + receivePercentComplete.ToString());
        }
